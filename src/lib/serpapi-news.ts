...
/**
 * Fetch a product image from SerpAPI Google News (first result thumbnail).
 * Use as alternative to Google Images for product hero image.
 */
export async function fetchProductImageFromNews(
  productName: string
): Promise<string | null> {
  const { items } = await fetchRelatedNews(productName);
  const thumbnail = items[0]?.thumbnail?.trim();
  return thumbnail && thumbnail.startsWith("http") ? thumbnail : null;
}
