  ...
  for (const m of filtered) {
    ...
    result.push(
      <a
        key={key++}
        href={`#${m.id}`}
        className="font-medium text-indigo-400 hover:underline hover:text-indigo-500"
      >
        {m.text}
      </a>
    );
...
export function BuySpectrum({ score, label = "Buy Now Score", explanation }: BuySpectrumProps) {
  ...
  const displayScore = score != null ? Math.round(value).toString() : "—";
    ...
    <div className="w-full rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
      ...
      <div className="mb-4 flex items-baseline gap-2">
        <span className="text-4xl font-bold text-zinc-900">
          {displayScore}
          {score != null && <span className="text-[0.5em] font-normal text-zinc-500">/100</span>}
        </span>
      </div>
      ...
      <div className="mt-6 rounded-xl bg-zinc-50 p-4">
        <p className={`text-lg font-semibold transition-colors duration-500 ${verdict.color}`}>{verdict.text}</p>
